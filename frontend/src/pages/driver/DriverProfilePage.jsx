          ...
          <form onSubmit={handleSubmit}>
              ...
              <div className="space-y-5 animate-fade-in">
                ...
                <div className="grid sm:grid-cols-2 gap-5">
                  ...
                  <div>
                    ...
                    <input name="age" type="number" min="18" value={form.age} onChange={handleChange} placeholder="Your age"
                ...
                <div className="pt-2">
                  ...
                  <input value={user?.email || ''} readOnly className="input-field bg-ink-50 dark:bg-ink-800/50 text-ink-400 cursor-not-allowed" />
                </div>
              </div>
            )}
            ...
            {activeTab === 1 && (
              <div className="space-y-5 animate-fade-in">
                ...
                <div>
                  ...
                  <div className="relative">
                    <Truck size={15} className="absolute left-3.5 top-1/2 -translate-y-1/2 text-ink-400" />
                    <select name="vehicleType" value={form.vehicleType} onChange={handleChange}
                      className={`input-field pl-10 appearance-none ${errors.vehicleType ? 'border-red-400' : ''}`}>
                ...
                <div className="grid sm:grid-cols-2 gap-5">
                  <div>
                    ...
                    <div className="relative">
                      <Hash size={15} className="absolute left-3.5 top-1/2 -translate-y-1/2 text-ink-400" />
                      <input name="vehicleNumber" value={form.vehicleNumber} onChange={handleChange} placeholder="DL01AB1234"
                        className={`input-field pl-10 uppercase ${errors.vehicleNumber ? 'border-red-400' : ''}`} />
                    </div>
                  ...
                  <div>
                    ...
                    <div className="relative">
                      ...
                      <input name="licenseNumber" value={form.licenseNumber} onChange={handleChange} placeholder="DL-2020-XXXXXXX"
                        className={`input-field pl-10 ${errors.licenseNumber ? 'border-red-400' : ''}`} />
                    </div>
                ...
                <div>
                  ...
                  <div className="relative">
                    <Package size={15} className="absolute left-3.5 top-1/2 -translate-y-1/2 text-ink-400" />
                    <input name="loadCapacity" value={form.loadCapacity} onChange={handleChange} placeholder="e.g., 5 Ton"
                      className={`input-field pl-10 ${errors.loadCapacity ? 'border-red-400' : ''}`} />
                  </div>
                  {errors.loadCapacity && <p className="text-xs text-red-500 mt-1">{errors.loadCapacity}</p>}
                </div>
              </div>
            )}
            ...
            {activeTab === 2 && (
              <div className="space-y-5 animate-fade-in">
                ...
                <div>
                  <label className="label">Quick Select Route</label>
                  <select value={newRoute} onChange={e => setNewRoute(e.target.value)}
                    className="input-field">
                    <option value="">Select a preset route...</option>
                    {PRESET_ROUTES.filter(r => !form.routes.includes(r)).map(r => <option key={r} value={r}>{r}</option>)}
                  </select>
                </div>
                ...
                <div className="flex gap-3">
                  <div className="flex-1">
                    ...
                    <input type="number" value={newFare} onChange={e => setNewFare(e.target.value)} placeholder="e.g., 1900"
                      className="input-field" min="0" />
                  </div>
                  <div className="flex items-end">
                    <button type="button" onClick={addRoute} disabled={!newRoute || !newFare}
                    ...
                    {form.routes.map(route => (
                      <div key={route} className="flex items-center justify-between gap-3 p-3.5 rounded-xl bg-ink-50 dark:bg-ink-800 border border-ink-100 dark:border-ink-700">
                        <div className="flex items-center gap-2 min-w-0">
                          ...
                          <span className="text-sm font-medium text-ink-800 dark:text-ink-200 truncate">{route}</span>
                        </div>
            ...
            <div className="flex gap-3 mt-8">
              {activeTab > 0 && (
                <button type="button" onClick={() => setActiveTab(t => t - 1)} className="btn-secondary flex-1">
                  ← Back
                </button>
              )}
              {activeTab < 2 ? (
                <button type="button" onClick={() => setActiveTab(t => t + 1)} className="btn-primary flex-1 flex items-center justify-center gap-2">
